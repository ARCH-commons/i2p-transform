...
/* Replace PCORNet ICD10 diagnoses hierarchy with the local hierarchy filling in
the pcornet_basecode with the expected values.
*/

create or replace view icd10_diag_local_to_pcori as
select '\PCORI\DIAGNOSIS\10\' as pcori_path
     , '\i2b2\Diagnoses\ICD10\' as local_path
     , 'ICD10:' as pcori_scheme
     , 'KUH|DX_ID:' as local_scheme
from dual;

insert into "&&i2b2_meta_schema".PCORNET_DIAG
with terms_dxi as (
  select 
    cicd.code dxicd, ht.*, m.*
  from "&&i2b2_meta_schema"."&&terms_table" ht
  join icd10_diag_local_to_pcori m 
    on ht.c_fullname like m.local_path||'%'
  -- TODO: Stop cheating by going back to Clarity
  left join clarity.edg_current_icd10@id cicd 
         on to_char(cicd.dx_id) = replace(ht.c_basecode, m.local_scheme, '')
  order by ht.c_hlevel
  )

select
  td.c_hlevel, 
  replace(td.c_fullname, local_path, pcori_path) c_fullname, 
  td.c_name, td.c_synonym_cd, td.c_visualattributes,
  td.c_totalnum, td.c_basecode, td.c_metadataxml, td.c_facttablecolumn, td.c_tablename, 
  td.c_columnname, td.c_columndatatype, td.c_operator, td.c_dimcode, td.c_comment, 
  td.c_tooltip, td.m_applied_path, td.update_date, td.download_date, td.import_date, 
  ...
  case 
    when td.dxicd is not null then td.dxicd
    when td.c_basecode like pcori_scheme||'%' then replace(td.c_basecode, pcori_scheme, '')
    else null 
  end pcori_basecode
from terms_dxi td
order by c_hlevel

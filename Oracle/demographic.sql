...
--1 --  S,R,NH
    ...
    select '1', patient_num, birth_date, to_char(birth_date,''HH24:MI''), sex.pcori_basecode, 'NI', 'NI', 'NI', race.pcori_basecode
    from i2b2patient p
...
union -- A - S,R,H
    select insert into demographic(raw_sex,PATID, BIRTH_DATE, BIRTH_TIME,SEX, SEXUAL_ORIENTATION, GENDER_IDENTITY, HISPANIC, RACE)
    select 'A', patient_num, birth_date, to_char(birth_date,''HH24:MI''), sex.pcori_basecode, 'NI', 'NI', hisp.pcori_basecode, race.pcori_basecode
    from i2b2patient p
    where lower(p.sex_cd) in (lower(sex.c_dimcode))
    and lower(p.race_cd) in (lower(race.c_dimcode))
...
union --2 S, nR, nH
    select insert into demographic(raw_sex,PATID, BIRTH_DATE, BIRTH_TIME,SEX, SEXUAL_ORIENTATION, GENDER_IDENTITY, HISPANIC, RACE)
    select '2', patient_num, birth_date, to_char(birth_date,''HH24:MI''), sex.pcori_basecode, 'NI', 'NI', 'NI', 'NI'
    from i2b2patient p
...
union --3 -- nS,R, NH
    select insert into demographic(raw_sex,PATID, BIRTH_DATE, BIRTH_TIME,SEX, SEXUAL_ORIENTATION, GENDER_IDENTITY, HISPANIC, RACE)
    select '3', patient_num, birth_date, to_char(birth_date,''HH24:MI''), 'NI', 'NI', 'NI', 'NI', race.pcori_basecode
    from i2b2patient p
...
union --B -- nS,R, H
    select insert into demographic(raw_sex,PATID, BIRTH_DATE, BIRTH_TIME,SEX, SEXUAL_ORIENTATION, GENDER_IDENTITY, HISPANIC, RACE)
    select 'B', patient_num, birth_date, to_char(birth_date,'HH24:MI'), 'NI', 'NI', 'NI', hisp.pcori_basecode, race.pcori_basecode
    from i2b2patient p
    where lower(nvl(p.sex_cd,'xx')) not in (select lower(code) from pcornet_codelist where codetype='SEX')
    and lower(p.race_cd) in (lower(race.c_dimcode))
    and lower(p.ethnicity_cd) in (lower(hisp.c_dimcode))
...
union --4 -- S, NR, H
    select insert into demographic(raw_sex,PATID, BIRTH_DATE, BIRTH_TIME,SEX, SEXUAL_ORIENTATION, GENDER_IDENTITY, HISPANIC, RACE)
    select '4', patient_num, birth_date, to_char(birth_date,''HH24:MI''), sex.pcori_basecode, 'NI', 'NI', hisp.pcori_basecode, 'NI'
    from i2b2patient p
...
union --5 -- NS, NR, H
    select insert into demographic(raw_sex,PATID, BIRTH_DATE, BIRTH_TIME,SEX, SEXUAL_ORIENTATION, GENDER_IDENTITY, HISPANIC, RACE)
    select '5', patient_num,  birth_date,  to_char(birth_date,'HH24:MI'), 'NI', 'NI', 'NI', hisp.pcori_basecode, 'NI'
    from i2b2patient p
    where lower(nvl(p.sex_cd,'xx')) not in (select lower(code) from pcornet_codelist where codetype='SEX')
    and lower(nvl(p.race_cd,'xx')) not in (select lower(code) from pcornet_codelist where codetype='RACE')
    and lower(nvl(p.ethnicity_cd,'NI')) in (lower(hisp.c_dimcode))
...
union --6 -- NS, NR, nH
    select insert into demographic(raw_sex,PATID, BIRTH_DATE, BIRTH_TIME,SEX, SEXUAL_ORIENTATION, GENDER_IDENTITY, HISPANIC, RACE)
    select '6', patient_num, birth_date,  to_char(birth_date,'HH24:MI'), 'NI', 'NI', 'NI', 'NI', 'NI'
    from i2b2patient p
    where lower(nvl(p.sex_cd,'xx')) not in (select lower(code) from pcornet_codelist where codetype='SEX')
    and lower(nvl(p.ethnicity_cd,'xx')) not in (select lower(code) from pcornet_codelist where codetype='HISPANIC')
    and lower(nvl(p.race_cd,'xx')) not in (select lower(code) from pcornet_codelist where codetype='RACE')
...
END;
/
--SELECT count(PATID) from demographic where rownum = 1

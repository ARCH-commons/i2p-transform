...
-- Create empty patient list table so views can be defined
...
IF  EXISTS (SELECT * FROM sys.views WHERE object_id = OBJECT_ID('i2b2visit')) DROP VIEW i2b2visit
...
-- You will almost certainly need to edit your database name
-- Change start_date here to transform a different window than 1/1/10-present
...
-- This is now ooptional, if you have not run the loyalty cohort it will create an empty view
...
----------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------
-- 7. LAB_RESULT_CM - by Jeff Klann, PhD and Arturo Torres
----------------------------------------------------------------------------------------------------------------------------------------
------------------------- LAB_RESULT_CM------------------------------------------------ 
...
CASE WHEN lab.pcori_specimen_source like '%SR_PLS' THEN 'SER_PLAS' WHEN lab.pcori_specimen_source like 'BLOOD' THEN 'BLD'  WHEN lab.pcori_specimen_source is null then 'NI' WHEN lab.pcori_specimen_source='' THEN 'NI' ELSE lab.pcori_specimen_source  END specimen_source, --bug fix with help from Snehil Gupta from Washington University 
...
INSERT INTO [dbo].[pmnharvest]([NETWORKID], [NETWORK_NAME], [DATAMARTID], [DATAMART_NAME], [DATAMART_PLATFORM], [CDM_VERSION], [DATAMART_CLAIMS], [DATAMART_EHR], [BIRTH_DATE_MGMT], [ENR_START_DATE_MGMT], [ENR_END_DATE_MGMT], [ADMIT_DATE_MGMT], [DISCHARGE_DATE_MGMT], [PX_DATE_MGMT], [RX_ORDER_DATE_MGMT], [RX_START_DATE_MGMT], [RX_END_DATE_MGMT], [DISPENSE_DATE_MGMT], [LAB_ORDER_DATE_MGMT], [SPECIMEN_DATE_MGMT], [RESULT_DATE_MGMT], [MEASURE_DATE_MGMT], [ONSET_DATE_MGMT], [REPORT_DATE_MGMT], [RESOLVE_DATE_MGMT], [PRO_DATE_MGMT], [REFRESH_DEMOGRAPHIC_DATE], [REFRESH_ENROLLMENT_DATE], [REFRESH_ENCOUNTER_DATE], [REFRESH_DIAGNOSIS_DATE], [REFRESH_PROCEDURES_DATE], [REFRESH_VITAL_DATE], [REFRESH_DISPENSING_DATE], [REFRESH_LAB_RESULT_CM_DATE], [REFRESH_CONDITION_DATE], [REFRESH_PRO_CM_DATE], [REFRESH_PRESCRIBING_DATE], [REFRESH_PCORNET_TRIAL_DATE], [REFRESH_DEATH_DATE], [REFRESH_DEATH_CAUSE_DATE], [MEDADMIN_START_DATE_MGMT], [MEDADMIN_STOP_DATE_MGMT], [REFRESH_MED_ADMIN_DATE], [REFRESH_OBS_CLIN_DATE], [REFRESH_PROVIDER_DATE], [REFRESH_OBS_GEN_DATE], [DEATH_DATE_MGMT], [OBSCLIN_DATE_MGMT], [OBSGEN_DATE_MGMT]) 
	VALUES('C1', 'SCILHS', dbo.getDataMartID(), dbo.getDataMartName(), dbo.getDataMartPlatform(), 4.1, '01', '02', '01','01','02','01','02','01','02','01','02','01','01','02','02','01','01','01','02','01',getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),getdate(),'01','02',getdate(),getdate(),getdate(),getdate(), '01', '01', '01')
...
insert into pmnprescribing (
    ...
    ,RAW_RX_MED_NAME
    ,RX_DOSE_FORM --Renamed from RX_QUANTITY_UNIT v4.0 --valueset changed for rxnorm terminology
    ,RX_SOURCE --hardcoded to OD
...
select distinct  m.patient_num, m.Encounter_Num,m.provider_id,  m.start_date order_date,  substring(convert(varchar,m.start_date,8),1,5), m.start_date start_date, m.end_date, mo.pcori_cui
    ,quantity.nval_num quantity, refills.nval_num refills, supply.nval_num supply, substring(freq.pcori_basecode,charindex(':',freq.pcori_basecode)+1,2) frequency, substring(basis.pcori_basecode,charindex(':',basis.pcori_basecode)+1,2) basis, substring(mo.c_name,0,50), unit.pcori_basecode, 'OD'
...
-- TODO: The first encounter inner join seems to slow things down
    ...
    IF OBJECT_ID('vw_pmnprescribing_loader') IS NOT NULL DROP VIEW vw_pmnprescribing_loader
    GO
...
-- @kval - The percentile of low-fact patients to eliminate (e.g., k=6 will eliminate all patients in the bottom 5% of the fact count)
-- @xnum - The max number of pts to transform. Specify for testing.
----------------------------------------------------------------------------------------------------------------------------------------
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'pcornetprep') AND type in (N'P', N'PC')) DROP PROCEDURE pcornetprep
go

create procedure pcornetprep 
    @kval INT=6,
    @xnum INT=100000000 
as
begin
...
SET @SQL='delete from i2b2patient_list where patient_num in 
    (select patient_num from
        (select patient_num, k, sum(f) f from
          ...
          ) t group by PATIENT_NUM,k) t where k<'+cast(@kval as varchar)+')';
EXEC(@SQL);

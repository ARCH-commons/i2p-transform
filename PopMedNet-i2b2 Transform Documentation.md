...
**ADD_SCILHS_100\MSSQL\*:** The SCILHS Annotated Data Dictionary Reporting Tool: A SQL Server version of a Python script to populate a summary spreadsheet of your PopMedNet tables that we will need to submit to PCORnet in January 2015.

**PMNTransform_v540\MSSQL\PCORNetLoader_v6.1.sql:** SQL Server versions of a script to re-create PopMedNet tables, prepare the transform, and load the stored procedures. Several bug fixes and speed optimizations (see changelog).
...
4. From your new PopMedNet database, open the PCORNetLoader_v6.sql script. Note that the entire transform (including create tables) is now in this single script. Edit the preamble to match your local setup, according to the instructions in the file. This now includes the following:
...
* The prescribing transform is slower than desired or expected at present.
